...
function proceedWithAction(actionType, param) {
    ...
    else if (actionType === 'word') exportToWord(); // <--- تمت الإضافة هنا
...
function runSmartOnboardingTour(forceStart = false) {
    ...
    const steps = [
        ...
        { selector: '#hashtagSearchInput', title: 'البحث والفلترة 🔍', text: 'اكتب اي كلمة تريد البحث عنها (مثل الفصل_الأول) لفلترة الأسئلة وقراءتها في وضع آمن، وعند مسح البحث يعود المحرر لطبيعته.' },
...
function getStrictCompactBubbleSheetContent(lType, sColor, modelName, placement) {
  ...
  // (استبدل أسطر الـ QR بهذا السطر ليعود كما كان)
    let modelHeaderHtml = modelName ? `<div style="text-align:center; margin-bottom: 8px;"><span style="border: 2px dashed ${sColor}; padding: 4px 20px; font-weight: 900; border-radius: 8px; color: ${sColor}; font-size: 15px; background: rgba(255,255,255,0.8);">${modelName}</span></div>` : ``;
...
/* ========================================================
   الميزات الإضافية الجديدة (Word, Voice, Hashtag Filter)
   ======================================================== */

// 1. ميزة الإملاء الصوتي (Voice to Text)
let speechRecog = null;
function toggleSpeechRecognition(targetId, btnEl) {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
        showToast('متصفحك لا يدعم الإملاء الصوتي. يرجى استخدام متصفح Google Chrome.', 'error');
        return;
    }
    ...
    speechRecog.lang = currentQuestionSystem === 'foreign' ? 'en-US' : 'ar-SA';
    ...
    speechRecog.onstart = function() {
        ...
        showToast('جاري الاستماع... تحدث الآن لسرد أسئلتك.', 'info');
    };
    ...
    speechRecog.onerror = function(event) {
        ...
        if (event.error === 'not-allowed') {
            errorMsg = '❌ المتصفح يمنع الميكروفون! يرجى الضغط على (علامة القفل 🔒) بجوار رابط الموقع في الأعلى والسماح للميكروفون.';
        } else if (event.error === 'no-speech') {
            errorMsg = '⚠️ لم أسمع شيئاً! الرجاء التحدث بصوت أعلى وأوضح.';
        } else if (event.error === 'network') {
            errorMsg = '❌ الإملاء الصوتي يتطلب اتصالاً بالإنترنت لكي يعمل.';
        }
...
// 2. ميزة البحث النصي الشامل في البنك
...
function filterQuestions(keyword) {
    ...
    if (!isFiltering) {
        originalQuestionsHTML = editor.innerHTML;
        isFiltering = true;
    }

    if (keyword.trim() === "") {
        editor.innerHTML = originalQuestionsHTML;
        isFiltering = false;
    ...
    let filteredHTML = `<div style="background: #fef3c7; color: #b45309; padding: 10px; border-radius: 6px; font-weight: bold; margin-bottom: 15px; text-align: center; font-size: 13px;">⚠️ وضع البحث نشط (للقراءة فقط). امسح مربع البحث للعودة للتعديل بحرية.</div>`;
    filteredHTML += getRawPreamble('questionsInput'); 
    ...
    questionsDatabase.forEach(q => {
        ...
        // البحث داخل نص السؤال
        if (q.text && q.text.toLowerCase().includes(searchWord)) isMatch = true;
        // البحث داخل الخيارات
        if (q.options) {
            q.options.forEach(o => {
                if (o.t && o.t.toLowerCase().includes(searchWord)) isMatch = true;
            });
        }
        // البحث داخل الإجابة
        if (q.ans && q.ans.toLowerCase().includes(searchWord)) isMatch = true;

        if (isMatch) {
            foundCount++;
            filteredHTML += `<br><div style="font-weight:900; color: var(--primary-color);">${q.num}. ${q.text.replace(/\n/g, '<br>')}</div>`;
            if (q.options) {
                q.options.forEach(o => {
                    let isCorrect = (q.ans === o.l || q.ans === o.t) ? ' [✓]' : '';
                    filteredHTML += ` ${o.l}) ${o.t} <span style="color:#10b981; font-weight:bold;">${isCorrect}</span><br>`;
                });
            }
            if (q.type === 'essay' && q.ans) {
                filteredHTML += `<div style="color:#10b981; font-weight:bold;">الإجابة: ${q.ans}</div>`;
            }
            filteredHTML += `<hr style="border-top:1px dashed #cbd5e1; margin: 10px 0;">`;
        }
    });

    if (foundCount === 0) {
        filteredHTML += `<div style="text-align: center; color: #ef4444; font-weight: bold; padding: 20px;">لم يتم العثور على نتائج للكلمة (${keyword}).</div>`;
    }

    editor.innerHTML = filteredHTML;
}
